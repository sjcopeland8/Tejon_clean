...
df_herb <- data_summary(Tick, varname="total",groupnames="plot")
...
df_climate <- data_summary(Tick, varname="total",groupnames="site")
...
herb3 <- ggplot(Tick_no0, aes(x = plot, y = log(1 + deoc), fill = plot)) +
  geom_boxplot() +
  theme_bw() +
  labs(x = 'Herbivory Treatment', y = 'Log D. occidentalis counts') +
  xtitletheme + ytitletheme +
  scale_fill_manual(values = c('darkseagreen1', 'darkseagreen3', 'darkseagreen4')) +
  guides(fill = FALSE) +
  scale_x_discrete(breaks = c("Open", "Partial", "Total"),
